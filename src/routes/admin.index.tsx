...
import { users, subjects, documents, escalations, subjectActivity, topicVolume } from "@/lib/mock-data";
...
function AdminDashboard() {
  ...
  const totalQueries = subjectActivity.reduce((s, a) => s + a.queries, 0);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <StatCard label="Total Students" value={students} caption={`${faculty} faculty · ${admin} admin`} color="indigo" />
        <StatCard label="Subjects" value={subjects.length} color="amber" />
        <StatCard label="Total Queries Asked" value={totalQueries} color="green" />
      ...
      <div className="mt-6">
        <Card title="Subject Activity">
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[11px] font-semibold uppercase tracking-wider text-slate-400">
                <th className="pb-3">Subject</th><th className="pb-3">Approved Docs</th><th className="pb-3">Student Queries</th>
              </tr>
            </thead>
            <tbody>
              {subjectActivity.map((s) => (
                <tr key={s.subject} className="border-t border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="py-3 text-slate-600">{s.queries}</td>
                </tr>
              ))}
            </tbody>
          </table>
