      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="flex items-center gap-2 text-xs">
          ...
          <select
            ...
            className="rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 outline-none focus:border-indigo-brand"
          >
          ...
          <Card key={p.id}>
            <div className="flex items-start gap-4">
              ...
              <button
                ...
                className="flex flex-col items-center justify-center rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-slate-600 hover:border-indigo-brand hover:bg-indigo-brand/5 hover:text-indigo-brand transition shrink-0"
              >
              ...
              <div className="flex-1 space-y-3 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-400">
                      Posted by <strong className="text-slate-700">{p.author}</strong> ({p.authorRole}) · {p.createdAt}
                    </span>
                  </div>
                </div>
                ...
                {p.answers.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-slate-100 space-y-3">
                    <div className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-1">
                      <MessageSquare className="h-3.5 w-3.5 text-indigo-brand" /> Answers ({p.answers.length})
                    </div>
                    {p.answers.map((ans) => (
                      <div
                        key={ans.id}
                      ...
                      >
                        <div className="flex items-center justify-between font-bold text-slate-800">
                          ...
                          <span className="text-[10px] text-slate-400 font-normal">{ans.createdAt}</span>
                        </div>
                        <p className="text-slate-700 leading-relaxed font-sans">{ans.content}</p>
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div className="mt-3 flex items-center gap-2 pt-2 border-t border-slate-100">
                  <input
                    ...
                    className="flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2 text-xs outline-none focus:border-indigo-brand focus:bg-white"
                  />
                  <button
                    type="button"
                    onClick={() => handleAddAnswer(p.id)}
                    disabled={!answerInput[p.id]?.trim()}
                    className="flex items-center gap-1 rounded-xl bg-indigo-brand px-4 py-2 text-xs font-semibold text-white hover:bg-indigo-brand-hover disabled:opacity-50 transition"
                  >
          ...
          <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2 font-serif text-lg font-bold text-slate-900">
                <MessageSquare className="h-5 w-5 text-indigo-brand" /> Ask a Question in Forum
              </div>
              <button
                type="button"
